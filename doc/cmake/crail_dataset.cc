  ...
  public:
    Dataset(OpKernelContext *ctx, std::vector<string> filenames,
        ...
        : DatasetBase(DatasetContext(ctx)), filenames_(std::move(filenames)),
          ...
          use_compression_(!compression_type.empty()), options_(options) {}
  ...
  private:
    ...
    private:
      ...
      std::unique_ptr<io::RandomAccessInputStream>
          input_stream_ GUARDED_BY(mu_);
      std::unique_ptr<io::ZlibInputStream> zlib_input_stream_ GUARDED_BY(mu_);
      std::unique_ptr<io::BufferedInputStream>
          buffered_input_stream_ GUARDED_BY(mu_);
      size_t current_file_index_ GUARDED_BY(mu_) = 0;
      std::unique_ptr<RandomAccessFile>
          file_ GUARDED_BY(mu_); // must outlive input_stream_
    };
